...
func TestCollectorError(t *testing.T) {
	testCollector(t, NewDomainCollector(whois.NewClient(), time.Second, safeconfig.Domain{Name: "fake.foo", Host: ""}), func(t *testing.T, status int, body string) {
...
func TestNotExpired(t *testing.T) {
	testCollector(
		t,
		NewDomainCollector(whois.NewClient(), time.Second, safeconfig.Domain{Name: "goreleaser.com", Host: ""}),
